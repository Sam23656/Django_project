...
function AllLanguagesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => 
    {
      ...
      const userData = await GetAllLanguages(true, id);
      setData(userData);
    ...
    <div>
      <div className='d-flex'>
        <div style={{ width: "100%" }} className="d-flex flex-column align-items-center flex-wrap">
          <h1 className="ms-3 me-3">Все языки</h1>
          ...
          <div style={{ marginTop: "140px", width: "100%" }} className="d-flex flex-wrap justify-content-center border-primary rounded-3">
            {data.results.map((language, index) => (
            ...
            <nav aria-label="..." className=''>
              <ul className="pagination">
                <li className={`page-item ${id === 1 ? 'disabled' : ''}`}>
                  <a className="page-link" href={`?location=Languages&page=${id - 1}`}>Previous</a>
                </li>
                {pages.map((page) => (
                  <li key={page} className={`page-item ${page === id ? 'active' : ''}`}>
                    <a className="page-link" href={`?location=Languages&page=${page}`}>
                      {page}
                    </a>
                  </li>
                ))}
                <li className={`page-item ${id === pages.length ? 'disabled' : ''}`}>
                  <a className="page-link" href={`?location=Languages&page=${id + 1}`}>Next</a>
                </li>
              </ul>
            </nav>
          )}
